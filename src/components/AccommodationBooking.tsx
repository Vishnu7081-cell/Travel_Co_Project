...
import { accommodationBookingsAPI } from "../utils/api";

interface AccommodationBookingProps {
  route: any; // contains destination info
  tripId: string; // comes from RoutePlanner save
...
export function AccommodationBooking({
  ...
  // 🔥 SAVE ACCOMMODATION BOOKING TO DB
  const handleBooking = async (accommodation: any) => {
    try {
      ...
      if (!tripId) {
        alert("Trip not found. Please start again.");
        return;
      }
      ...
      const checkIn = new Date(route.travelDate);
      const checkOut = new Date(route.travelDate);
      checkOut.setDate(checkOut.getDate() + 1);

      const response = await accommodationBookingsAPI.create({
        tripId: tripId,
        customerId: storedUser.id,
        hotelName: accommodation.name,
        roomType: "Deluxe", // Using valid enum value
        checkInDate: checkIn,
        checkOutDate: checkOut,
        pricePerNight: accommodation.price,
        nights: 1,
        totalPrice: accommodation.price,
        starRating: Math.round((accommodation.rating / 1) * 1) / 1, // rating is already 1-5
      });
      ...
      console.log("✅ Accommodation saved:", bookingData);

      // Pass booking record forward
      onSelect({
        ...accommodation,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-6 shadow-md border border-gray-100">
        ...
        <p className="text-gray-600">
          Stay in{" "}
          <b>
            {route?.destinationDistrict || "your destination"}
          </b>
        </p>
      </div>
          ...
          {["all", "hotel", "resort", "guesthouse"].map(
            (filter) => (
              <button
                ...
                className={`px-6 py-3 rounded-lg whitespace-nowrap transition-all ${selectedFilter === filter
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    Central{" "}
                    {route?.destinationDistrict || "City"}
                  </p>
                </div>
